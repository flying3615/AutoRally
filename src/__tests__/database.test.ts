...
function setupSchema(db: Database) {
  ...
  db.exec(`
    ...
    CREATE TABLE games (id TEXT PRIMARY KEY, sessionId TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, courtNumber INTEGER NOT NULL, team1Player1Id TEXT NOT NULL REFERENCES players(id) ON DELETE CASCADE, team1Player2Id TEXT NOT NULL REFERENCES players(id) ON DELETE CASCADE, team2Player1Id TEXT NOT NULL REFERENCES players(id) ON DELETE CASCADE, team2Player2Id TEXT NOT NULL REFERENCES players(id) ON DELETE CASCADE, status TEXT NOT NULL CHECK(status IN ('pending', 'playing', 'completed')), roundNumber INTEGER NOT NULL, gameType TEXT NOT NULL CHECK(gameType IN ('mixed', 'male-double', 'female-double', 'open-double')), startedAt TEXT, endedAt TEXT);
    ...
    CREATE TABLE payments (id TEXT PRIMARY KEY, playerId TEXT NOT NULL REFERENCES players(id) ON DELETE CASCADE, sessionId TEXT REFERENCES sessions(id) ON DELETE CASCADE, amount REAL NOT NULL, status TEXT NOT NULL CHECK(status IN ('paid', 'unpaid')), paidDate TEXT, paymentType TEXT NOT NULL CHECK(paymentType IN ('session', 'topup')));
